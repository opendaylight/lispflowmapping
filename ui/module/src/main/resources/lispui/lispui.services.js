define(['app/lispui/lispui.module'], function(lispui) {
    ...
    lispui.register.factory('LispuiNodeFormSvc', ['$filter',
        ...
        function($filter, $location, apiBuilder, constants,
            ...
            api.generateNodesToApis = function(callback,
                errorCbk) {
                var allRootNodes = [],
                    topLevelSync = syncFact.generateObj(),
                    reqAll = topLevelSync.spawnRequest(
                        'all');
                    ...
                    function(data) {
                        ...
                        yangUtils.processModules(
                            ...
                            function(result) {
                                ...
                                topLevelSync.removeRequest(
                                    reqAll);
                            });
                    },
                    function(result) {
                        console.error(
                            'Error getting API data:',
                            result);
                        topLevelSync.removeRequest(
                            reqAll);
                    }
                ...
                topLevelSync.waitFor(function() {
                    try {
                        eventDispatcher.dispatch(
                            constants.EV_SRC_MAIN,
                            'Building apis');
                        var abApis = apiBuilder.processAllRootNodes(
                            allRootNodes);
                        callback(abApis,
                            allRootNodes);
                    } catch(e) {
                        ...
                        .customOperation(operation.toLowerCase(),
                            ...
                            function(data) {
                                ...
                                if(data) {
                                    ...
                                    props.forEach(
                                        function(p) { //fill each property - needed for root mountpoint node, in other cases there should be only one property anyway
                                            scope.node
                                                .fill(
                                                    p,
                                                    data[
    ...
    lispui.register.factory('LispuiDashboardSvc', ['LispuiNodeFormSvc',
        ...
        function(LispuiNodeFormSvc, LispuiRestangular,
            ...
            api.deleteKey = function(eid) {
                return api.getSingleKey(eid).then(function(
                    key) {
                    var postKey = {
                        "input": {}
                    };
                    postKey.input.LispAddressContainer =
                        key.LispAddressContainer;
                    postKey.input['mask-length'] =
                        key['mask-length'];
            ...
            api.deleteMapping = function(eid) {
                return api.getSingleMapping(eid).then(
                    function(mapping) {
                        var postMapping = {
                            "input": {}
                        };
                        postMapping.input.LispAddressContainer =
                            mapping.LispAddressContainer;
                        postMapping.input['mask-length'] =
                            mapping['maskLength'];
