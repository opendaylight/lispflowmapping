...
final class MappingServiceIntegrationTestUtil {
        ...
        for(MapRequest mapRequest : mapRequests) {
            ...
            if (counterDecremented < 0) {
                fail("checkSmr: SMR contains EID " + LispAddressStringifier.getString(smrEid) +
                        ", which is not in the list of expected EIDs: " + eidSet);
            } else {
     ...
     * @return a HasMap of Eid objects and the number of their occurrences
     */
    static HashMap<Eid, Integer> prepareExpectedEids(long vni, String ... eids) {
        final HashMap<Eid, Integer> eidSet = new HashMap<>();
        for (String eidString : eids) {
            Eid eid = LispAddressUtil.asIpv4Eid(eidString, vni);
            eidSet.putIfAbsent(eid, 0);
            int counterIncremented = eidSet.get(eid) + 1;
            eidSet.put(eid, counterIncremented);
        }
        return eidSet;
    }
