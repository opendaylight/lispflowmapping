...
public final class MappingMergeUtil {
    ...
    private static void mergeCommonMappingRecordFields(MappingRecordBuilder mrb, MappingRecord record) {
        // Set xTR-ID and site-ID from the current mapping, it help with
    ...
    private static void mergeLocatorRecords(MappingRecordBuilder mrb, MappingRecord newRecord) {
        ...
        // Optimization: unless we had to merge any of the locators due to
        // differences in weights, etc, we return
        // the original 'locators' List with any new locators added
        boolean mergeHappened = false;

        // We assume locators are unique and don't show up several times (with
        // different or identical p/w/mp/mw),
        // so we create a LinkedHashMap (which preserves order) of the locators
        // from the existing merged record,
        // keyed by the Rloc
        ...
        for (LocatorRecord newLocator : newRecord.getLocatorRecord()) {
            ...
            if (locatorMap.containsKey(newRloc)) {
                // XXX LocatorRecord YANG generated class doesn't override
                // equals() so I'm not sure of the behavior
                ...
                // We add both the LinkedHanshMap and the List, in case we can
                // return the original list plus new
                // elements and need not generate a new list with merged
                // locators (which should be the most common
                // scenario).
        ...
        if (mergeHappened) {
            ...
            // TODO Check if this is necessary after and .add() was called on
            // locators
    ...
    public static MappingRecord mergeXtrIdMappings(List<Object> records, List<byte[]> expiredMappings,
        ...
        for (int i = 1; i < records.size(); i++) {
            ...
            // Skip expired mappings and add them to a list to be returned to
            // the caller
    ...
    public static Object computeNbSbIntersection(MappingRecord nbMapping, MappingRecord sbMapping) {
        // returns a MappingRecord which has the more specific EID, and
        // intersection of locator records.
        ...
        if (MaskUtil.isMaskable(sbMapping.getEid().getAddress())
            ...
            if (nbMapping.getEid().getAddress() instanceof SourceDestKey) {
                if (SourceDestKeyHelper.getDstMask(nbMapping.getEid()) < sbMapping.getMaskLength()) {
                    // We have to create a new SourceDest EID, where the source
                    // is same as
                    // the one in NB record, and dest EID is the more specific
                    // from SB
                    // mapping record.
            ...
            } else if (nbMapping.getMaskLength() < sbMapping.getMaskLength()) {
                // Both EIDs are IP prefixes.
                // SB mapping is a subprefix so we have to update EID
                // intersection
    ...
    private static List<LocatorRecord> getCommonLocatorRecords(MappingRecord nbMapping, MappingRecord sbMapping) {
        // This method updates the MappingRecord builder with the intersection
        // of the locator records from the two mappings. NB mapping records
        // fields have precedence, only Priority is updated from SB mapping if p
        // is 255.
        ...
        // We assume locators are unique and don't show up several times (with
        // different or identical p/w/mp/mw),
        // so we create a HashMap of the locators from the SB mapping record,
        // keyed by the Rloc
        ...
        // Gradually building final list of common locators, in order that they
        // appear in NB Mapping
        ...
        for (LocatorRecord nbLocator : nbMapping.getLocatorRecord()) {
            ...
            if (sbLocatorMap.containsKey(nbRloc)) {
                // common locator found. use the NB record as the common
                // locator.

                if (sbLocatorMap.get(nbRloc).getPriority() == (short) 255) {
                    // if SB locator has p == 255 then common locator takes all
                    // NB fields
