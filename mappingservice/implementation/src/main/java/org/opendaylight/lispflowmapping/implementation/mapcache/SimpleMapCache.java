...
public class SimpleMapCache implements IMapCache {
    ...
    public void addMapping(Eid key, Object subkey, Object value, boolean shouldOverwrite) {
        ...
        if (!shouldOverwrite) {
            ILispDAO xtrIdDao = getOrInstantiateGenericSubTable(eid, table);
            xtrIdDao.put(subkey, new MappingEntry<>(SubKeys.REGDATE, timestamp));
            xtrIdDao.put(subkey, new MappingEntry<>(SubKeys.RECORD, value));
        }
    }
    ...
    public void removeMapping(Eid eid, Object subkey, boolean overwrite) {
        ...
        if (!overwrite) {
            ILispDAO xtrIdTable = getGenericSubTable(key, table);
            if (xtrIdTable != null) {
                xtrIdTable.removeSpecific(subkey, SubKeys.RECORD);
            }
        }
    }
