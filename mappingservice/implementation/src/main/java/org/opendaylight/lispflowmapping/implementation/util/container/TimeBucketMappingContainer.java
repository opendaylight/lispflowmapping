...
public class TimeBucketMappingContainer {
    ...
    public int add(Eid key, String subKey, ILispDAO containerDao) {
        bucketList.get(current).add(key, subKey, containerDao);
        return current;
    }

    public int refreshMappping(Eid key, String subKey, ILispDAO dao, int containerId) {
        bucketList.get(containerId).remove(key, subKey);
        return add(key, subKey, dao);
    }
