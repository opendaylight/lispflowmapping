	...
	public MapRegister deserialize(ByteBuffer registerBuffer) {
        try {
            ...
            if (authenticationLength != authentication.getAuthenticationLength()) {
            	throw new LispSerializationException(String.format("Authentication length isn't correct. Expected:%d, got %d",authentication.getAuthenticationLength(), authenticationLength));
            }
