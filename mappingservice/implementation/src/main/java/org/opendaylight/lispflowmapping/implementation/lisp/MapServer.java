    ...
    public void handleMapRegister(MapRegister mapRegister, boolean smr, IMapNotifyHandler callback) {
        if (dao == null) {
            ...
            for (EidToLocatorRecord eidRecord : mapRegister.getEidToLocatorRecord()) {
                ...
                boolean mappingChanged = DAOMappingUtil.saveRlocs(eidRecord, smr, dao, shouldIterateMask(), shouldOverwrite());
                if (smr && mappingChanged) {
                    Set<SubscriberRLOC> subscribers = DAOMappingUtil.getSubscribers(eidRecord.getLispAddressContainer(),
                            eidRecord.getMaskLength(), dao);
