...
public class MapServer implements IMapServer {
    ...
    public MapNotify handleMapRegister(MapRegister mapRegister) {
        ...
        IMappingServiceKey key = MappingServiceKeyFactory.getInstance().generateMappingServiceKey(eidRecord.getPrefix(), eidRecord.getMaskLength());
        dao.put(key, entry);
    ...
    private String getPassword(LispAddress prefix, int maskLength) {
        ...
        while (maskLength > 0) {
            IMappingServiceKey key = MappingServiceKeyFactory.getInstance().generateMappingServiceKey(prefix, maskLength);
    ...
    public boolean removeAuthenticationKey(LispAddress address, int maskLen) {
        IMappingServiceKey key = MappingServiceKeyFactory.getInstance().generateMappingServiceKey(address, maskLen);
        Map<String, ?> daoMap = dao.get(key);
        if (daoMap != null) {
            MappingServiceValue value = (MappingServiceValue) daoMap.get("value");
            if (value != null) {
    ...
    public boolean addAuthenticationKey(LispAddress address, int maskLen, String key) {
        IMappingServiceKey mappingServiceKey = MappingServiceKeyFactory.getInstance().generateMappingServiceKey(address, maskLen);
