...
public class LispMappingService implements CommandProvider, IFlowMapping, BindingAwareConsumer, NotificationListener<LispNotification> {
    ...
    private ConsumerContext guymember;
    ...
    public void onSessionInitialized(ConsumerContext session) {
        ...
        if (lispSB == null) {
            logger.error("Lisp SB wasn't found, something bad is going on");
        }*/
        this.session = session;
        //  session.getRpcService(ILSP.class);
    }
    ...
    public void onNotification(LispNotification notification) {
        try {
            ...
            if (lispMessage instanceof MapRequest) {
                logger.info("MapRequest NOTIFICAITON RECIEVED!");
                ...
                logger.info("MapRegister NOTIFICAITON RECIEVED!");
                MapNotify mapNotify = handleMapRegister((MapRegister) lispMessage);
                // lispSB.handleMapNotify(mapNotify);
    ...
    public void guy(ConsumerContext context) {
        this.guymember = context;
    }
}
