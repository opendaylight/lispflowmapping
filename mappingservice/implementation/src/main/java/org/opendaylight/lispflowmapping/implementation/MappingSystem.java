...
public class MappingSystem implements IMappingSystem {
    ...
    public void addMapping(MappingOrigin origin, Eid key, MappingData mapping, boolean merge) {
        if (mapping == null) {
        ...
        if (origin == MappingOrigin.Southbound) {
            ...
            if (xtrId != null && merge) {
                ...
                MappingData mergedEntry = MappingMergeUtil.mergeXtrIdMappings(smc.getAllXtrIdMappings(key),
    ...
    public void refreshMappingRegistration(Eid key, XtrId xtrId, Long timestamp) {
        ...
        if (mapping != null) {
            mapping.setTimestamp(new Date(timestamp));
        }
        if (xtrId != null) {
            MappingData xtrIdMapping = (MappingData) smc.getMapping(key, xtrId);
            if (xtrIdMapping != null) {
                xtrIdMapping.setTimestamp(new Date(timestamp));
            }
        }
    }

    private MappingData updateServicePathMappingRecord(MappingData mapping, Eid eid) {
    ...
    public MappingData getMapping(Eid src, Eid dst, XtrId xtrId) {
        ...
        MappingData xtrIdMapping = (MappingData) smc.getMapping(dst, xtrId);
        if (MappingMergeUtil.mappingIsExpired(xtrIdMapping)) {
            smc.removeMapping(dst, xtrId);
            return null;
        } else {
            return xtrIdMapping;
        }
    ...
    private MappingData getMappingNbFirst(Eid src, Eid dst) {
        ...
        MappingData nbMapping = (MappingData) pmc.getMapping(src, dst);
    ...
    private MappingData getMappingNbSbIntersection(Eid src, Eid dst) {
        ...
        MappingData sbMapping = getSbMappingWithExpiration(src, dst);
        if (sbMapping == null) {
            return nbMapping;
        }
    ...
    private MappingData getSbMappingWithExpiration(Eid src, Eid dst) {
        MappingData mapping = (MappingData) smc.getMapping(src, dst);
        if (MappingMergeUtil.mappingIsExpired(mapping)) {
            smc.removeMapping(dst);
    ...
    private void restoreDaoFromDatastore() {
        ...
        for (Mapping mapping : mappings) {
            ...
            addMapping(mapping.getOrigin(), mapping.getMappingRecord().getEid(),
                    new MappingData(mapping.getMappingRecord()), false);
        }
