...
public class MappingServiceMockitoTest {
    ...
    private static final RpcResult RPC_RESULT_SUCCESS = RpcResultBuilder.success().build();
    private static final MappingAuthkey MAPPING_AUTHKEY = new MappingAuthkeyBuilder().setKeyString("password").build();
    ...
    public void addKeyTest() throws ExecutionException, InterruptedException {
        ...
        // input
        final RpcResult rpc = RpcResultBuilder.failed().withError(RpcError.ErrorType.PROTOCOL, "data-exists",
                "Key already exists! Please use update-key if you want to change it.").build();
        final RpcError error = (RpcError) rpc.getErrors().iterator().next(); // equals() not implemented int RpcError
     ...
     */
    @Test
    public void getKeyTest_withNullMapAuthkey() throws ExecutionException, InterruptedException {
        // input
        final GetKeyInput getKeyInput = new GetKeyInputBuilder().setEid(IPV4_EID).build();
        Mockito.when(mappingSystem.getAuthenticationKey(getKeyInput.getEid())).thenReturn(null);

        final RpcResult rpc = RpcResultBuilder.failed().withError(RpcError.ErrorType.APPLICATION, "data-missing",
                "Key was not found in the mapping database").build();
        final RpcError error = (RpcError) rpc.getErrors().iterator().next();
    ...
    public void getMappingTest() throws ExecutionException, InterruptedException {
        ...
        final RpcResult rpcResult = result.get();
    ...
    public void updateKeyTest_withNullMapAuthkey() throws ExecutionException, InterruptedException {
        ...
        // input
        final RpcResult rpc = RpcResultBuilder.failed().withError(RpcError.ErrorType.PROTOCOL, "data-missing",
