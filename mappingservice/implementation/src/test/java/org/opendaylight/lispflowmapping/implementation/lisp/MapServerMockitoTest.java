...
public class MapServerMockitoTest {
    ...
    private static final SubscriberRLOC SUBSCRIBER_RLOC = new SubscriberRLOC(RLOC,             // timedOut() == true
            IPV4_SOURCE_EID_1, new Date(86400000L * 2));
    private static final SubscriberRLOC SUBSCRIBER_RLOC_2 = new SubscriberRLOC(RLOC_2,         // timedOut() == false
            IPV4_SOURCE_EID_2);
    private static final Eid SOURCE_DEST_KEY_EID = LispAddressUtil.asSrcDstEid(IPV4_STRING, IPV4_STRING_2, MASK, MASK, VNI);
    ...
    public void handleMapRegisterTest_MappingMergeFalse() throws NoSuchFieldException, IllegalAccessException {
        ...
        assertEquals(1, subscriberSetMock.size());
    }
    ...
    public void handleMapRegisterTest_verifyTransportAddresses() {
        ...
        Mockito.when(mapService.getData(MappingOrigin.Southbound, IPV4_EID, SubKeys.SUBSCRIBERS))
                .thenReturn(subscriberSetMock);
        Mockito.when(mapService.getData(MappingOrigin.Southbound, IPV4_EID, SubKeys.SRC_RLOCS))
    ...
    public void handleMapRegisterTest_withTwoMappingRecords() {
        ...
        // TODO remove next line after fix
        mapRegisterSrcDstBuilder.setMappingRecordItem(new ArrayList<>());
        ...
        Mockito.when(mapService.getMapping(MappingOrigin.Southbound, SOURCE_DEST_KEY_EID))
                // make sure mappings are different
                .thenReturn(OLD_MAPPING_RECORD)
                .thenReturn(OLD_MAPPING_RECORD_2)
                .thenReturn(OLD_MAPPING_RECORD_2);
        Mockito.when(mapService.getData(MappingOrigin.Southbound, IPV4_EID, SubKeys.SUBSCRIBERS))
        ...
        Mockito.when(mapService.getData(MappingOrigin.Southbound, SourceDestKeyHelper.getDst(SOURCE_DEST_KEY_EID),
                SubKeys.SUBSCRIBERS)).thenReturn(subscriberSetMock);
        // TODO add second subscriberSet

        // result
        ArgumentCaptor<MapRequest> captor = ArgumentCaptor.forClass(MapRequest.class);
        mapServer.handleMapRegister(mapRegisterSrcDstBuilder.build());
        Mockito.verify(notifyHandler, Mockito.times(1)).handleSMR(captor.capture(), Mockito.eq(RLOC_2));

        final Eid resultEid = captor.getValue().getEidItem().iterator().next().getEid();
        assertEquals(IPV4_SOURCE_EID_2, resultEid);
    }

    @Test
    @SuppressWarnings("unchecked")
    public void mappingChangedTest_withDifferentEid() {
    ...
    @Test
    @SuppressWarnings("unchecked")
    public void mappingChangedTest_withDifferentRLOC() {
        final MappingRecordBuilder mappingRecordBuilder_1 = getDefaultMappingRecordBuilder();
        // apply the change
    ...
    @Test
    @SuppressWarnings("unchecked")
    public void mappingChangedTest_withDifferentAction() {
        final MappingRecordBuilder mappingRecordBuilder_1 = getDefaultMappingRecordBuilder()
                // apply the change
    ...
    @Test
    @SuppressWarnings("unchecked")
    public void mappingChangedTest_withDifferentTTL() {
        final MappingRecordBuilder mappingRecordBuilder_1 = getDefaultMappingRecordBuilder()
                // apply the change
                .setRecordTtl(10);
        final MappingRecordBuilder mappingRecordBuilder_2 = getDefaultMappingRecordBuilder();
    ...
    @Test
    @SuppressWarnings("unchecked")
    public void mappingChangedTest_withDifferentMapVersion() {
        final MappingRecordBuilder mappingRecordBuilder_1 = getDefaultMappingRecordBuilder()
                // apply the change
    ...
    @Test
    @SuppressWarnings("unchecked")
    public void mappingChangedTest_withNullMap() {
        final MappingRecordBuilder mappingRecordBuilder_2 = getDefaultMappingRecordBuilder();
        final ArgumentCaptor<List> captor = ArgumentCaptor.forClass(List.class);
