...
public class MapServerMockitoTest {
    ...
    public void handleMapRegisterTest_MappingMergeFalse() throws NoSuchFieldException, IllegalAccessException {
        setConfigInitMappingMergeField(false);
    ...
    public void handleMapRegisterTest_MappingMergeTrue() throws NoSuchFieldException, IllegalAccessException {
        setConfigInitMappingMergeField(true);
        final MapRegister mapRegister = getDefaultMapRegisterBuilder().build();
        final MappingRecord oldMappingRecord = getDefaultMappingRecordBuilder().setEid(IPV4_EID_2).build();
    ...
    private static void setConfigInitMappingMergeField(boolean value) throws NoSuchFieldException, IllegalAccessException {
        ConfigIni configIni = ConfigIni.getInstance();
        final Field mappingMergeField = configIni.getClass().getDeclaredField("mappingMerge");
        mappingMergeField.setAccessible(true);
        mappingMergeField.setBoolean(configIni, value);
    }
}
