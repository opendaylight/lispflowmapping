...
import org.opendaylight.lispflowmapping.implementation.util.timebucket.containers.TimeBucket;
...
/**
 * Created by sheikahm on 12/13/16.
 */
...
public class TimeBucketWheelUnitTest {
    ...
    private void checkOlderThanCurrentCase(TimeBucketWheel timeBucketWheel) {
        ...
        Mockito.verify(mappingSystem).handleExpiredMappingInTimeBucket(IPV4_EID_2, toBeExpiredMappingData);
    }
    ...
    private MappingData getMappingDataFromTimeBucket(TimeBucket timeBucket, Eid eid) {
        HashMap<Eid, MappingData> bucketElements;

        try {
            bucketElements = (HashMap<Eid, MappingData>) Whitebox.getInternalState(timeBucket,
                    "bucketElements");
        } catch (ClassCastException e) {
            throw e;
        }
    ...
    public void expiredMappingClearedProperlyTest() {
        ...
        Mockito.verify(mappingSystem).handleExpiredMappingInTimeBucket(IPV4_EID_1,
                mappingData);
    }
