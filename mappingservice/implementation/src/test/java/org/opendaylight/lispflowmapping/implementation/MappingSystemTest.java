...
public class MappingSystemTest {
    ...
    public void getMappingTest_NbFirst_withServicePathDestinationAddress_singleELPLocatorRecord() {
        ...
        MappingRecord result = (MappingRecord) mappingSystem.getMapping(EID_IPV4_SRC, EID_SERVICE_PATH);
        Ipv4 ipv4Result = (Ipv4) result.getLocatorRecord().get(0).getRloc().getAddress();
        assertEquals(IPV4_STRING_3, ipv4Result.getIpv4().getValue());
    }
    ...
    /**
     * Tests {@link MappingSystem#getMapping} method with dst address.
     */
    @Test
    ...
    private static Rloc getIpPrefixTypeRloc() {
        ...
        final Address address = new Ipv4PrefixBuilder().setIpv4Prefix(new org.opendaylight.yang.gen.v1.urn.ietf.params.xml
                .ns.yang.ietf.inet.types.rev100924.Ipv4Prefix(IPV4_STRING_1 + MASK)).build();
