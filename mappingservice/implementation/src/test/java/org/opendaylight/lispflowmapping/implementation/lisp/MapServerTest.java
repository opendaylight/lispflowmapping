...
public class MapServerTest extends BaseTestCase {
    ...
    private void addDAOEpectations(LispAddress address, int mask, int withoutPassword, int withPassword, String password) {
        ...
        allowing(lispDAO).put(weq(MappingServiceKeyFactory.getInstance().generateMappingServiceKey(clonedAddress, (byte) mask)),
    ...
    private void addPasswordExpextations(LispAddress address, int mask, int withoutPassword, int withPassword, String password) {
        ...
        if (withoutPassword > 0) {
            ...
            allowing(lispDAO).getSpecific(
                    with(MappingServiceKeyFactory.getInstance().generateMappingServiceKey(clonedAddress, (byte) withoutPassword)), with("value"));
            ret(result);
        }
        clonedAddress = (LispAddress) ((IMaskable) address).clone();
        if (withPassword > 0) {
            ...
            allowing(lispDAO).get(with(MappingServiceKeyFactory.getInstance().generateMappingServiceKey(clonedAddress, (byte) withPassword)));
            ret(result);
        }
        for (int i = mask; i >= 0; i--) {
            clonedAddress = (LispAddress) ((IMaskable) address).clone();
            allowing(lispDAO).get(with(MappingServiceKeyFactory.getInstance().generateMappingServiceKey(clonedAddress, (byte) i)));
    ...
    private IMappingServiceKey getKey(int mask) {
        IMappingServiceKey key = MappingServiceKeyFactory.getInstance().generateMappingServiceKey(eid, mask);
        return key;
    }
