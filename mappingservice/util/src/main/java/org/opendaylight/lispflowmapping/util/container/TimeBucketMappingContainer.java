package org.opendaylight.lispflowmapping.util.container;
...
public class TimeBucketMappingContainer {
    ...
    //TODO proper type
    private List<HashMap<Object, Object>> bucketList;
    ...
    private void clearSpecificBucket(int bucketIndex) {
        HashMap<Object, Object> bucket = bucketList.get(bucketIndex);

        bucket.forEach((key, mapping) -> {
            //TODO clear the bucket
        });
    ...
    public int add(Object key, Object value) {
        bucketList.get(current).put(key, value);
        return current;
    }

    public int refreshMappping(Object key, int containerId) {
        Object record = bucketList.get(containerId).get(key);
        bucketList.get(containerId).remove(key);
        return add(key, record);
    }
