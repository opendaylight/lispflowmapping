...
public class TimeBucketMappingContainer {
    ...
    public int add(Object key, String subKey, MappingAllInfo mappingAllInfo) {
        bucketList.get(current).add(key, subKey, mappingAllInfo);
        return current;
    }

    public int refreshMappping(Object key, String subKey, MappingAllInfo mappingAllInfo, int containerId) {
        TimeBucket timeBucket = bucketList.get(containerId);
        timeBucket.remove(key, subKey);
