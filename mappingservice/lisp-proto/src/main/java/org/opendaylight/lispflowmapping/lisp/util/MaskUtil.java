...
public class MaskUtil {
    ...
    public static Eid normalize(Eid eid) {
        ...
        try {
            if (address instanceof Ipv4Prefix) {
                String[] v4prefix = ((Ipv4Prefix)address).getIpv4Prefix().getValue().split("/");
                ...
                String[] v6prefix = ((Ipv6Prefix)address).getIpv6Prefix().getValue().split("/");
    ...
    public static short getMaskForIpPrefix(IpPrefix prefix) {
        return Short.parseShort(prefix.getValue().toString().split("/")[1]);
    }
