...
public class MapRequestSerializer {
    ...
    public MapRequest deserialize(ByteBuffer requestBuffer) {
        try {
            ...
            LispAddressSerializerContext ctx = LispAddressSerializer.DEFAULT_EID_CTX;
            builder.setSourceEid(new SourceEidBuilder().setEid(
                    (Eid) LispAddressSerializer.getInstance().deserialize(requestBuffer, ctx)).build());

            if (builder.getItrRloc() == null) {
                builder.setItrRloc(new ArrayList<ItrRloc>());
            }
            ctx = LispAddressSerializer.DEFAULT_RLOC_CTX;
            for (int i = 0; i < itrCount; i++) {
                builder.getItrRloc().add(new ItrRlocBuilder().setRloc(
                        (Rloc) LispAddressSerializer.getInstance().deserialize(requestBuffer, ctx)).build());
            }
