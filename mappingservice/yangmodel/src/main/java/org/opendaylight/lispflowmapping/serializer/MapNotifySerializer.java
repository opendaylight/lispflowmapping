...
public class MapNotifySerializer {
    ...
    public ByteBuffer serialize(MapNotify mapNotify) {
        ...
        if (mapNotify.isXtrSiteIdPresent() != null && mapNotify.isXtrSiteIdPresent()) {
            size += org.opendaylight.lispflowmapping.serializer.MapRegisterSerializer.Length.XTRID_SIZE +
                    org.opendaylight.lispflowmapping.serializer.MapRegisterSerializer.Length.SITEID_SIZE;
        }
    ...
    public MapNotify deserialize(ByteBuffer notifyBuffer) {
        try {
            ...
            if (xtrSiteIdPresent) {
                byte[] xtrId  = new byte[org.opendaylight.lispflowmapping.serializer.MapRegisterSerializer.Length.XTRID_SIZE];
                notifyBuffer.get(xtrId);
                byte[] siteId = new byte[org.opendaylight.lispflowmapping.serializer.MapRegisterSerializer.Length.SITEID_SIZE];
