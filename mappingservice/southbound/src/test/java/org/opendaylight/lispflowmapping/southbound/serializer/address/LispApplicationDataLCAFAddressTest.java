...
public class LispApplicationDataLCAFAddressTest extends BaseTestCase {
    ...
    public void deserialize__Simple() throws Exception {
        ...
        assertEquals(new LispIpv4Address(0x11223344), appAddress.getApplicationData().getAddress());
        ArrayAssert.assertEquals(new byte[]{(byte) 0xAA,(byte) 0xBB,(byte) 0xCC}, appAddress.getApplicationData().getIPTos());
        assertEquals((byte)0xDD, appAddress.getApplicationData().getProtocol());
        assertEquals((short)0xA6A1, appAddress.getApplicationData().getLocalPort());
        assertEquals((short)0xFFDD, appAddress.getApplicationData().getRemotePort());
    ...
    public void deserialize__Ipv6() throws Exception {
    	LispApplicationDataLCAFAddress appAddress = (LispApplicationDataLCAFAddress) LispAddressSerializer.getInstance().deserialize(hexToByteBuffer("40 03 00 00 " + //
                "04 20 00 0A " + //
                "AA BB CC DD " + // IPTOS & protocol
                "A6 A1 FF DD " + // local port & remote port
        ...
        assertEquals(new LispIpv6Address(new byte[] { 0x11, 0x22, 0x33, 0x44, //
                ...
                (byte) 0xAA, (byte) 0xBB, (byte) 0xCC, (byte) 0xDD }), appAddress.getApplicationData().getAddress());
    ...
    public void serialize__Simple() throws Exception {
		...
		appData.setIPTos(new byte[] {(byte)0xAA, (byte)0xBB,(byte)0xCC});
		appData.setProtocol((byte)0xDD);
		appData.setLocalPort((short)0xA6A1);
		appData.setRemotePort((short)0xFFDD);
		appData.setAddress(new LispIpv4Address(0x11223344));
        LispApplicationDataLCAFAddress address = new LispApplicationDataLCAFAddress((byte) 0x20, appData);
