...
import org.opendaylight.yang.gen.v1.urn.opendaylight.lfm.lisp.sb.rev150904.ctrl.msg.stats.control.message.CountersBuilder;
...
public class LispSouthboundRPC implements LispSbService {
    ...
    public Future<RpcResult<Void>> sendMapNotify(SendMapNotifyInput mapNotifyInput) {
        ...
        if (mapNotifyInput != null) {
            ...
            lispSbPlugin.handleSerializedLispBuffer(mapNotifyInput.getTransportAddress(), outBuffer, LispType.MapNotify);
        } else {
    ...
    public Future<RpcResult<Void>> sendMapReply(SendMapReplyInput mapReplyInput) {
        ...
        if (mapReplyInput != null) {
            ...
            lispSbPlugin.handleSerializedLispBuffer(mapReplyInput.getTransportAddress(), outBuffer, LispType.MapReply);
        } else {
    ...
    public Future<RpcResult<Void>> sendMapRequest(SendMapRequestInput mapRequestInput) {
        ...
        if (mapRequestInput != null) {
            ...
            lispSbPlugin.handleSerializedLispBuffer(mapRequestInput.getTransportAddress(), outBuffer, LispType.MapRequest);
        } else {
    ...
    public Future<RpcResult<Void>> sendMapRegister(SendMapRegisterInput mapRegisterInput) {
        ...
        if (mapRegisterInput != null) {
            ...
            lispSbPlugin.handleSerializedLispBuffer(mapRegisterInput.getTransportAddress(), outBuffer, LispType.MapRegister);
        } else {
    ...
    private static GetStatsOutput createGetStatsOutput(LispSouthboundStats stats) {
        ...
        for (int i = 0; i <= LispSouthboundStats.MAX_LISP_TYPES; i++) {
            if (LispType.forValue(i) == null) {
                continue;
            }
            ControlMessageBuilder cmb = new ControlMessageBuilder();
            cmb.setType(LispType.forValue(i));
            CountersBuilder cb = new CountersBuilder();
            cb.setRxCount(rxStats[i]);
            cb.setTxCount(txStats[i]);
            cmb.setCounters(cb.build());
