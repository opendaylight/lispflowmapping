...
public class LispSouthboundHandler extends SimpleChannelInboundHandler<DatagramPacket>
    ...
    private MapRegisterCacheValue refreshAuthKeyIfNecessary(MapRegisterCacheValue mapRegisterCacheValue) {
        if (lispSbPlugin.getAuthenticationKeyDataListener().isAuthKeyRefreshing()) {
            final boolean shouldAuthKeyRefreshingStop = System.currentTimeMillis()
                    - lispSbPlugin.getAuthenticationKeyDataListener().getAuthKeyRefreshingDate()
                    > lispSbPlugin.getMapRegisterCacheTimeout();
            if (shouldAuthKeyRefreshingStop) {
                lispSbPlugin.getAuthenticationKeyDataListener().setAuthKeyRefreshing(false);
            } else {
