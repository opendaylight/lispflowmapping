...
public class LispSouthboundHandler extends SimpleChannelInboundHandler<DatagramPacket>
    ...
    private void handleMapRegister(ByteBuffer inBuffer, InetAddress sourceAddress, int port) {
        try {
            ...
            if (cacheEntry != null && mapRegisterCache.isMapRegisterInCacheAndCurrent(cacheEntry.getKey())) {
                ...
                if (cacheEntry.getValue().isWantMapNotifyBitSet()) {
                    sendNotification(sourceAddress, inBuffer, port);
                }
            } else {
    ...
    private void sendNotification(final InetAddress inetAddress, final ByteBuffer inBuffer, int portNumber) {
