...
public class LispSouthboundHandler extends SimpleChannelInboundHandler<DatagramPacket>
    ...
    private void responseImmediatelly(final ByteBuffer inBuffer) {

    }

    private List<Eid> provideEidPrefixesFromMessage(final MapRegister mapRegister) {
        List<Eid> eidsResult = new ArrayList<>();
        for (MappingRecordItem mappingRecordItem : mapRegister.getMappingRecordItem()) {
            eidsResult.add(mappingRecordItem.getMappingRecord().getEid());
        }
        return eidsResult;
    }
