...
public class LispSouthboundHandler extends SimpleChannelInboundHandler<DatagramPacket>
    ...
    private void handleMapRegister(ByteBuffer inBuffer, InetAddress sourceAddress, int port) {
        try {
            final Map.Entry<MapRegisterCacheKey, MapRegisterCacheValue> artificialEntry = MapRegisterPartialDeserializer
            ...
            if (cacheValue != null) {
                ...
                if (artificialEntry != null) {
                    final MapRegisterCacheValue originCacheValue = artificialEntry.getValue();
    ...
    private MapRegisterCacheValue resolveCacheValue(Map.Entry<MapRegisterCacheKey, MapRegisterCacheValue> entry) {
        if (entry != null) {
            ...
            if (mapRegisterCacheValue != null) {
                ...
                if (currentTime - creationTime <= CACHE_RECORD_TIMEOUT && Arrays.equals(mapRegisterCacheValue
                        .getPacketData(), entry.getValue().getPacketData())) {
