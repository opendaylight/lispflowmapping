...
public class LispSouthboundHandler extends SimpleChannelInboundHandler<DatagramPacket>
    ...
    private void handleMapRegister(ByteBuffer inBuffer, InetAddress sourceAddress, int port) {
        try {
            final Map.Entry<MapRegisterCacheKey, MapRegisterCacheValue> cacheEntry = MapRegisterPartialDeserializer
                    .deserializePartially(inBuffer, sourceAddress);
            if (cacheEntry != null && mapRegisterCache.isMapRegisterKeyInCache(cacheEntry.getKey()) &&
                    mapRegisterCache.isMapRegisterMsgCurrent(cacheEntry.getKey())) {
                    LOG.debug("Map register message {} was already processed.", cacheEntry.getKey());
                    mapRegisterCache.refreshRecord(cacheEntry.getKey());
                    sendNotificationIfPossible(createMappingKeepAlive(cacheEntry.getValue()));
                    if (cacheEntry.getValue().isWantMapNotifyBitSet()) {
                        sendMapNotify(sourceAddress, inBuffer, port);
                    }
            } else {
                ...
                if (cacheEntry != null) {
                    final MapRegisterCacheValue value = cacheEntry.getValue();
                    ...
                    mapRegisterCache.addMapRegisterEntryToCache(cacheEntry.getKey(), value);
    ...
    private void sendMapNotify(final InetAddress inetAddress, final ByteBuffer inBuffer, int portNumber) {
