 ...
 */
package org.opendaylight.lispflowmapping.neutron.vppneutron.manager;
...
import org.opendaylight.lispflowmapping.neutron.util.VppNodeReader;
...
public class VppNodeManager {
    ...
    public VppNodeManager(@Nonnull final DataBroker dataBroker,
        ...
        this.vppNodeReader = new VppNodeReader(mountPointService);
    }
    ...
    private void updateNode(final Node node) {
        ...
        final NetconfNode netconfNode = getNodeAugmentation(node);
        if (netconfNode == null) {
            LOG.debug("Node {} is not a netconf node", nodeId);
        }
    ...
    private void processConnectedNode(final Node node) {
        ...
        openstackNodeInformationManager.addRlocOfHost(node.getNodeId().getValue(), rlocOfComputeNode);
    }

    private void processDisconnectedNode(final Node node) {
        openstackNodeInformationManager.deleteAllEidsOfHostAndMarkUnprocessed(node.getNodeId().getValue());
    }
