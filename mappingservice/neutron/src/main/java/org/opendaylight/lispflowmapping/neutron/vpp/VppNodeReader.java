...
import org.opendaylight.lispflowmapping.neutron.vpp.exception.RlocNotFoundOnVppNode;
import org.opendaylight.lispflowmapping.neutron.vpp.util.VppNetconfTrasaction;
import org.opendaylight.vbd.impl.PPrint;
    ...
    public Ipv4Address rlocIpOfNode(KeyedInstanceIdentifier<Node, NodeKey> instanceIdentifierToVppNode) {
        try {
            ...
            if (ipv4AddressOptional.isPresent()) {
                throw new RlocNotFoundOnVppNode(PPrint.node(instanceIdentifierToVppNode));
            }

            return ipv4AddressOptional.get();
        } catch (final InterruptedException | ExecutionException ex) {
            LOG.warn("Got exception while reading IP addresses from nodes {}",
                    PPrint.node(instanceIdentifierToVppNode));
            throw new RlocNotFoundOnVppNode(PPrint.node(instanceIdentifierToVppNode));
        }
    }

    private ListenableFuture<Optional<Ipv4Address>>
        readFirstAvailableIpOfVppNode(final KeyedInstanceIdentifier instanceIdentifierToVppNode) {
