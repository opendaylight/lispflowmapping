...
import org.opendaylight.lispflowmapping.neutron.vppneutron.LispNeutronAsyncExecutorProvider;
import org.opendaylight.lispflowmapping.neutron.vppneutron.exception.RlocNotFoundOnVppNode;
...
public class VppNodeReader {
    ...
    public Ipv4Address rlocIpOfNode(KeyedInstanceIdentifier<Node, NodeKey> instanceIdentifierToVppNode) {
        try {
            Optional<Ipv4Address> ipv4AddressOptional =
                    readTenantIpOfVppNode(instanceIdentifierToVppNode).get();
            if (!ipv4AddressOptional.isPresent()) {
                throw new RlocNotFoundOnVppNode(NodeInfoUtil.getNodeName(instanceIdentifierToVppNode));
            }

            return ipv4AddressOptional.get();
        } catch (final InterruptedException | ExecutionException ex) {
            LOG.warn("Got exception while reading IP addresses from nodes {}",
                    NodeInfoUtil.getNodeName(instanceIdentifierToVppNode));
            throw new RlocNotFoundOnVppNode(NodeInfoUtil.getNodeName(instanceIdentifierToVppNode));
        }
    }

    private ListenableFuture<Optional<Ipv4Address>>
        readTenantIpOfVppNode(final KeyedInstanceIdentifier<Node, NodeKey> instanceIdentifierToVppNode) {
        ...
        return Futures.transformAsync(vppDataBrokerFuture, (DataBroker vppDataBroker) -> {
            ...
            if (vppDataBroker != null) {
                final Optional<InterfacesState> interfacesOnVppNodeOptional =
                        LfmVppNetconfTransaction.read(vppDataBroker, LogicalDatastoreType.OPERATIONAL,
                ...
                if (interfacesOnVppNodeOptional.isPresent()) {
                    ...
                    for (Interface intf : interfacesOnVppNodeOptional.get().getInterface()) {
                        ...
                        if (ipv4AddressOptional.isPresent()) {
                            ...
                            LOG.debug("Ethernet interface found with IP {} but without description. Setting it as "
                                    + "backup in case no other interface found", ipv4AddressOptional.get());
    ...
    private String readInterfaceDescription(DataBroker vppDataBroker, String interfaceName, String hostName) {
        ...
        Optional<org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.interfaces.rev140508.interfaces
                .Interface> interfaceInfoOpt = LfmVppNetconfTransaction.read(vppDataBroker,
    ...
    private Optional<Ipv4Address> readIpAddressFromInterface(Interface intf,
        ...
        if (ipv4 == null) {
            LOG.debug("Ipv4 address for interface {} on node {} is null!", augIntf, NodeInfoUtil.getNodeName(iiToVpp));
            return Optional.absent();
        }

        final List<Address> addresses = ipv4.getAddress();
        if (addresses == null || addresses.isEmpty()) {
            LOG.debug("Ipv4 addresses list is empty for interface {} on node {}", augIntf,
                    NodeInfoUtil.getNodeName(iiToVpp));
            return Optional.absent();
        }

        final Ipv4AddressNoZone ip = addresses.iterator().next().getIp();
        if (ip == null) {
            LOG.debug("Ipv4AddressNoZone is null for node {}", NodeInfoUtil.getNodeName(iiToVpp));
            return Optional.absent();
        }
