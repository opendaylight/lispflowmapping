...
*/
    ...
    private MappingAuthkey getAuthKeyLpm(Eid prefix, ILispDAO db) {
        Eid modifiedPrefix = prefix;
        if (prefix.getVirtualNetworkId() == null) {
            modifiedPrefix = new EidBuilder(prefix).setVirtualNetworkId(new InstanceIdType(0L)).build();
        }
        short maskLength = MaskUtil.getMaskForAddress(modifiedPrefix.getAddress());
        while (maskLength >= 0) {
            Eid key = MaskUtil.normalize(modifiedPrefix, maskLength);
